...
def sample_patch(im: torch.Tensor,
    ...
    if mode == 'inside' or mode == 'inside_major':
        pad_mode = 'replicate'
        im_sz = torch.Tensor([im.shape[2], im.shape[3]]).to(im.device)
    ...
    szl = torch.max(sz.round(), torch.Tensor([2]).to(im.device)).long()
    ...
    if mode == 'inside' or mode == 'inside_major':
        im2_sz = torch.LongTensor([im2.shape[2], im2.shape[3]]).to(im.device)
