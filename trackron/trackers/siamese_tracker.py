...
class SiameseTracker(BaseTracker):
    ...
    def init_sot(self, image, info: dict, **kwargs) -> dict:
        ...
        template_mask = torch.from_numpy(template_mask_arr).to(torch.bool).to(
            self.device)[None]
        template_box = self.get_cropped_img_box(
            self.pos, self.target_sz, self.pos,
            scale_factor).reshape(1, 4)  ## xyxy format
    ...
    def track_sot(self, image, info: dict = None) -> dict:
        ...
        proposal_box = self.get_cropped_img_box(
            self.pos, self.target_sz, self.pos,
            scale_factor).reshape(1, 4)  ## xyxy format
        ...
        self.target_box = self.find_target(search_tensor, search_mask,
                                           scale_factor)
    ...
    def find_target(self, search_tensor, search_mask, scale_factor):
        ...
        if results.get('pred_scores', 1.0) < -10:
            ...
            target_box = clip_box(self.map_box_back(pred_box, self.pos,
                                                    scale_factor),
                                  self.image_sz,
                                  margin=2)
        self.pos = torch.Tensor([
    ...
    def init_mot(self, image, info: dict, category=None) -> dict:
        ...
        if self.use_pub_detection:
            ...
            public_detections = torch.tensor(info['init_detections']).to(
                self.device)
            public_detections[:, :4] = normalize_boxes(
                public_detections[:, :4],
                image.shape[:2],
                in_format='xywh',
                out_format='cxcywh')
        ...
        self.mask_tensor = torch.zeros((1, *im_tensor.shape[-2:]),
                                       ...
                                       device=self.device)
        with torch.no_grad():
            outputs, mot_ref_info = self.net.track_mot(
                im_tensor, mask=self.mask_tensor, ref_info=self.mot_ref_info)
    ...
    def track_mot(self, image, info):
        ...
        if self.use_pub_detection:
            ...
            public_detections[:, :4] = normalize_boxes(
                ...
                out_format='cxcywh')
            self.mot_ref_info[
                'public_detections'] = public_detections.unsqueeze(
                    0)  ### batch dim
        with torch.no_grad():
            outputs, mot_ref_info = self.net.track_mot(
                im_tensor, mask=mask_tensor, ref_info=self.mot_ref_info)
        self.mot_ref_info = mot_ref_info
    ...
    def update_mot_memory(self, output_results, use_embedding=False):
        ...
        for idx in range(track_bboxes.shape[0]):
            if idx in self.tracks_dict and track_bboxes is not None:
                ...
                elif track_scores > self.track_thresh:
                    self.tracks_dict[idx]["bbox"] = track_bboxes[
                        idx, :].cpu().numpy().tolist()
        ...
        if N > 0 and M > 0:
            det_box = torch.stack(
                [torch.tensor(obj['bbox']) for obj in results], dim=0)  # N x 4
            track_box = torch.stack(
                [torch.tensor(obj['bbox']) for obj in tracks], dim=0)  # M x 4
            cost_bbox = 1.0 - generalized_box_iou(
                det_box, track_box).cpu().numpy()  # N x M
            # cost_bbox = 1.0 - ious(det_box.cpu().numpy(), track_box.cpu().numpy())

            matched_indices = linear_sum_assignment(cost_bbox)
            unmatched_dets = [
                d for d in range(N) if not (d in matched_indices[0])
            ]
            unmatched_tracks = [
                d for d in range(M) if not (d in matched_indices[1])
            ]
