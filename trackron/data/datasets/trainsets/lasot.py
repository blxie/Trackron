...
class Lasot(BaseVideoDataset):
    ...
    def __init__(self,
        ...
        self.sequence_resolution = json.load(
            open(os.path.join(data_path, 'data_specs',
                              'losot_resolution.json')))
    ...
    def _read_target_visible(self, seq_path):
        ...
        with open(occlusion_file, 'r', newline='') as f:
            occlusion = torch.ByteTensor(
                [int(v) for v in list(csv.reader(f))[0]])
        with open(out_of_view_file, 'r') as f:
            out_of_view = torch.ByteTensor(
                [int(v) for v in list(csv.reader(f))[0]])
    ...
    def _get_frame_path(self, seq_path, frame_id):
        return os.path.join(seq_path, 'img',
                            '{:08}.jpg'.format(frame_id +
                                               1))  # frames start from 1
