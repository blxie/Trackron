...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self,
        ...
        if split is not None:
            ...
            cur_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                                   '..')
            if split == 'train':
                file_path = os.path.join(cur_dir, 'data_specs',
    ...
    def _get_sequence_resolution(self, seq_id):
        height = self.sequence_meta_info[self.sequence_list[seq_id]]['height']
        width = self.sequence_meta_info[self.sequence_list[seq_id]]['width']
        return (height, width)

    def get_sequence_info(self, seq_id):
        seq_path = self._get_sequence_path(seq_id)
        bbox = self._read_bb_anno(seq_path)
        height, width = self._get_sequence_resolution(seq_id)
    ...
    def get_mot_frames(self, seq_id=None, frame_ids=None, anno=None):
        ...
        for fid in frame_ids:
            ann = {
                'track_id': seq_id,
                'class_name': obj_meta['object_class_name']
            }
            for key, value in anno.items():
