...
class Transform:
    ...
    def __call__(self, **inputs):
        ...
        for v in inputs.keys():
            if v not in self._valid_all:
                raise ValueError(
                    'Incorrect input \"{}\" to transform. Only supports inputs {} and arguments {}.'
                    .format(v, self._valid_inputs, self._valid_args))
    ...
    def _split_inputs(self, inputs):
        var_names = [k for k in inputs.keys() if k in self._valid_inputs]
        split_inputs = [{k: v
                         for k, v in zip(var_names, vals)}
                        for vals in zip(*[inputs[vn] for vn in var_names])]
        for arg_name, arg_val in filter(
                lambda it: it[0] != 'joint' and it[0] in self._valid_args,
                inputs.items()):
...
class TransformBase:
    ...
    def __call__(self, **inputs):
        ...
        input_vars = {
            k: v
            for k, v in inputs.items() if k in self._valid_inputs
        }
        ...
        for var_name, var in input_vars.items():
            if var is not None:
                ...
                if var_name in ['coords', 'bbox']:
                    params = (
                        self._get_image_size(input_vars), ) + self._rand_params
                else:
                    params = self._rand_params
                if isinstance(var, (list, tuple)):
                    outputs[var_name] = [
                        transform_func(x, *params) for x in var
                    ]
                else:
    ...
    def transform_bbox(self, bbox, image_shape, *rand_params):
        ...
        coord_transf = self.transform_coords(coord_all, image_shape,
                                             *rand_params).flip(0)
...
class RandomSelect(TransformBase):
    """
    ...
    def __call__(self, **inputs):
        ...
        if new_roll:
            ...
            assert getattr(self, 'rand', None) is not None
        if self.rand < self.p:
            return self.transforms1(**inputs)
        return self.transforms2(**inputs)
...
class Compose(TransformBase):
    ...
    def __call__(self, joint=True, new_roll=True, **inputs):
        for t in self.transforms:
            inputs = t(**inputs, joint=joint, new_roll=new_roll)
        return inputs
...
class ToTensorAndJitter(TransformBase):
    ...
    def roll(self):
        return np.random.uniform(max(0, 1 - self.brightness_jitter),
                                 1 + self.brightness_jitter)
...
class ToGrayscale(TransformBase):
    ...
    def __init__(self, probability=0.5):
        ...
        self.color_weights = np.array([0.2989, 0.5870, 0.1140],
                                      dtype=np.float32)
...
class RandomHorizontalFlip(TransformBase):
    ...
    def transform_bbox(self, boxes, image_shape, do_flip):
        """[summary]

    Args:
        boxes (List of Nx4): [in xywh format]
        image_shape ([h, w]): [description]
        do_flip ([type]): [description]
...
class RandomAffine(TransformBase):
    ...
    def transform_image(self, image, do_flip, theta, shear_values,
                        scale_factors):
        if torch.is_tensor(image):
    ...
    def transform_coords(self, coords, image_shape, do_flip, theta,
                         shear_values, scale_factors):
        t_mat = self._construct_t_mat(image_shape, do_flip, theta,
                                      shear_values, scale_factors)

        t_mat_tensor = torch.from_numpy(t_mat).float()

        coords_xy1 = torch.stack(
            (coords[1, :], coords[0, :], torch.ones_like(coords[1, :])))

        coords_xy_t = torch.mm(t_mat_tensor, coords_xy1)

        return coords_xy_t[[1, 0], :]

    def transform_mask(self, mask, do_flip, theta, shear_values,
                       scale_factors):
        t_mat = self._construct_t_mat(mask.shape[:2], do_flip, theta,
                                      shear_values, scale_factors)
        output_sz = (mask.shape[1] + 2 * self.pad_amount,
                     mask.shape[0] + 2 * self.pad_amount)

        mask_t = cv.warpAffine(mask.numpy(),
                               t_mat,
                               output_sz,
...
class ResizeShortestEdge(TransformBase):
    """
    ...
    def __init__(self,
        ...
        self._init(locals())
        self.max_size = 99999 if max_size is None else max_size
    ...
    def __call__(self, **inputs):
        ...
        # Roll random parameters for the transform
        outputs = dict()
        image_size = self._get_image_size(input_vars)
        if input_args.get('new_roll', True):
            self.resize = self._get_image_resize(image_size)

        assert self.resize is not None
        for var_name, var in input_vars.items():
            if var is not None:
                transform_func = getattr(self, 'transform_' + var_name)
                outputs[var_name] = transform_func(var, image_size,
                                                   self.resize)
        return outputs

    def _get_image_resize(self, image_sz):
        h, w = image_sz
        if self.is_range:
            size = np.random.randint(self.short_edege_length[0],
        ...
        neww = int(neww + 0.5)
        newh = int(newh + 0.5)
        # self.neww, self.newh = neww, newh
        return (newh, neww)

    def transform_image(self, image, image_shape, resize):
        newh, neww = resize
        if isinstance(image, (list, tuple)):
            image = [cv.resize(im, (neww, newh)) for im in image]
        else:
            image = cv.resize(image, (neww, newh))
        return image

    def transform_bbox(self, boxes, image_shape, resize):
        """Assumes List of [N,4] [x, y, w, h]"""
        h, w = image_shape
        newh, neww = resize
        h_scale = newh / h
        w_scale = neww / w
        boxes = [
            torch.stack([
                box[:, 0] * w_scale, box[:, 1] * h_scale, box[:, 2] * w_scale,
                box[:, 3] * h_scale
            ],
                        dim=-1) for box in boxes
        ]
        return boxes

    def transform_mask(self, masks_list, image_shape, resize):
        newh, neww = resize
        new_masks = []
        for mask in masks_list:
            if isinstance(mask, np.ndarray):
                ...
                raise ValueError("dtype must be np.ndarray or torch.Tensor")
            mask = F.interpolate(mask[None], (newh, neww))[0].bool()
            new_masks.append(mask)
        return new_masks


class RandomSizeCrop(TransformBase):
    """
    Scale the shorter edge to the given size, with a limit of `max_size` on the longer edge.
    If `max_size` is reached, then downscale so that the longer edge does not exceed max_size.
    """

    def __init__(self, min_size, max_size=1333):
        """
        Args:
            short_edge_length (list[int]): If ``sample_style=="range"``,
                a [min, max] interval from which to sample the shortest edge length.
    ...
    def __call__(self, **inputs):
        ...
        image_size = self._get_image_size(input_vars)
        if input_args.get('new_roll', True):
            h = random.randint(self.min_size, min(image_size[0],
                                                  self.max_size))
            w = random.randint(self.min_size, min(image_size[1],
                                                  self.max_size))
            self.crop_region = self._get_crop_region(image_size, (h, w))
        for var_name, var in input_vars.items():
            if var is not None:
                transform_func = getattr(self, 'transform_' + var_name)
                outputs[var_name] = transform_func(var, self.crop_region)
        return outputs

    def _get_crop_region(self, image_sz, crop_sz):
        h, w = image_sz
        th, tw = crop_sz
    ...
    def transform_image(self, image, crop_region):
        i, j, h, w = crop_region
        if isinstance(image, (list, tuple)):
            image = [im[i:i + h, j:j + w, :] for im in image]
        else:
            image = image[i:i + h, j:j + w, :]
        return image

    def transform_bbox(self, boxes, crop_region):
        """Assumes List of [N,4] [x, y, w, h]"""
        i, j, h, w = crop_region
        boxes = [
            torch.stack([box[:, 0] - j, box[:, 1] - i, box[:, 2], box[:, 3]],
                        dim=-1) for box in boxes
        ]
        return boxes

    def transform_att(self, att, crop_region):
        i, j, h, w = crop_region
        if isinstance(att, np.ndarray):
            att = torch.from_numpy(att).to(torch.bool)
        elif isinstance(att, torch.Tensor):
            att = att.to(torch.bool)
        else:
            raise ValueError("dtype must be np.ndarray or torch.Tensor")
        att = att[i:i + h, j:j + w]
        return att

    def transform_mask(self, mask_list, crop_region):
        i, j, h, w = crop_region
        new_masks = []
