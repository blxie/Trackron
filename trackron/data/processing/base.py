...
class SiamProcessing(SiameseBaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template']:
            ...
            # TODO: 将 SOT 的 crop 去掉，直接使用原图进操作
            jittered_boxes = [
            ...
            mask = F.interpolate(image_mask[None].float(),
        ...
        if self.box_mode == 'xyxy':
            data['search_boxes'] = box_convert(data['search_boxes'], 'xywh',
                                               'xyxy')
...
class SiamProcessingBRPadding(SiamProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        # TODO: 修改 search image 大小为全图，不进行裁剪操作
        # for s in ['template', 'search']:
        for s in ['template']:
            assert self.mode == 'sequence' or len(data[s + '_images']) == 1, \
                "In pair mode, num template/search frames must be 1"
