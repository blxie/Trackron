...
def align_and_update_state_dicts(model_state_dict, ckpt_state_dict):
    ...
    for idx_model, idx_ckpt in enumerate(idxs.tolist()):
        ...
        if key_ckpt in matched_keys:  # already added to matched_keys
            logger.error(
                ...
                "It matches at least two keys in the model ({} and {}).".
    ...
    original_keys = {
        k: original_keys[rev_matched_keys[k]]
        for k in matched_model_keys
    }
    ...
    for key_model in matched_model_keys:
        ...
        if key_model in model_key_groups:
            ...
            table.append((
                _longest_common_prefix([k[len(common_prefix):]
                                        for k in group]) + "*",
...
def _group_str(names: List[str]) -> str:
    ...
    ret = ret.replace("bn_beta,bn_running_mean,bn_running_var,bn_gamma",
                      "bn_*")
    return ret
