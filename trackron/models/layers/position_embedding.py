...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        if self.normalize:
            eps = 1e-6
            y_embed = y_embed / (y_embed[:, -1:, :] +
                                 eps) * self.scale  # 2pi * (y / sigma(y))
            x_embed = x_embed / (x_embed[:, :, -1:] +
                                 eps) * self.scale  # 2pi * (x / sigma(x))
...
class PositionEmbeddingNone(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        x = tensor_list.tensors
        b, _, h, w = x.size()
        return torch.zeros((b, self.n_dim, h, w),
                           device=x.device)  # (B, C, H, W)


def build_position_encoding(name, channel):
    N_steps = channel // 2
    if name in ('v2', 'sine'):
        # TODO find a better way of exposing other arguments
        position_embedding = PositionEmbeddingSine(N_steps, normalize=True)
    elif name in ('v3', 'learned'):
...
def get_feature_maskposemb(feat, mask, posemb_func):
    C, H, W = feat.shape[1:]
    ...
    pos_emb = posemb_func(nested_feat).to(feat.dtype)
    return mask, pos_emb
