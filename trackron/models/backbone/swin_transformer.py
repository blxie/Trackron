...
from trackron.models.layers import ShapeSpec, PatchEmbed
...
class SwinTransformer(Backbone):
    ...
    def __init__(self,
        ...
        self.output_layers = output_layers if output_layers else "layer{}".format(
            len(depths))
        ...
        out_feature_channels = {"patch": embed_dim}
        out_feature_strides = {"path": 4}

        # build layers
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            num_channel = int(embed_dim * 2**i_layer)
            layer = BasicLayer(
                dim=num_channel,
            ...
            out_feature_channels["layer{}".format(i_layer + 1)] = num_channel
            out_feature_strides["layer{}".format(i_layer + 1)] = 4 * 2**i_layer
    ...
    def forward(self, x, output_layers=None):
        ...
        if "patch" in self.output_layers:
            outputs["patch"] = x
        ...
        if self.ape:
            ...
            x = (x + absolute_pos_embed).flatten(2).transpose(1,
                                                              2)  # B Wh*Ww C
            poses.append(absolute_pos_embed)
        else:
...
def swin_trans_t(cfg):
    # def swin_trans_t(pretrained=False, ape=False, frozen_stages=-1):
    model = SwinTransformer(embed_dim=96,
                            ape=cfg.MODEL.BACKBONE.USE_POSITION,
