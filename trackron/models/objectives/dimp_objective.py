...
class DiMPObjective(BaseObjective):
    ...
    def from_config(cls, cfg):
        loss_func = {
            # TRACED: cls loss 初始化
            # 基于 trackron/config/objective_configs.py 中的配置
    ...
    def forward(self, results, data):
        ...
        cls_losses_search = [
            self.loss_func['cls_loss'](s, data['search_label'])
        ...
        if isinstance(self.loss_weight['cls_loss'], float) == 1:
            cls_ws = [
                ...
                for _ in range(len(cls_losses_search))
            ]
        else:
        ...
        if len(cls_losses_search) > 1:
            ...
            weighted_losses[
                'losses/loss_init_clf'] = cls_losses_search[0] * cls_ws[0]
            # Loss for the intermediate filter iterations
            losses['losses/loss_iter_clf'] = sum(
                [l for l in cls_losses_search])
