...
class SOTObjective(BaseObjective):
    ...
    def calculate_box_losses(self, pred_boxes, gt_boxes):
        ...
        l1_loss = self.loss_func['loss_bbox'](
            box_convert(pred_boxes, 'xyxy', 'cxcywh'),
            box_convert(gt_boxes, 'xyxy', 'cxcywh'),
            reduction='none')
...
class SOTObjectiveMultiQuery(SOTObjective):
    ...
    def forward(self, results, data):
        ...
        # box prediction use on-frame feature
        l1_loss = sum([
            self.loss_func['loss_bbox'](boxes, gt_boxes)
            for boxes in pred_boxes
        ]) / len(pred_boxes)
        try:
            giou_loss, iou = sum([
                self.loss_func['loss_giou'](boxes, gt_boxes)
                for boxes in pred_boxes
            ]) / len(pred_boxes)
        except:
            giou_loss, iou = torch.tensor(1.0).to(
...
class SequenceSOTObjective(SOTObjective):
    ...
    def forward(self, results, data, normalize_box=True):
        ...
        if pred_scores is not None and len(
                pred_scores) > 0 and giou_loss.item() < 1.0:
            center_loss = torch.zeros_like(iou)
            for boxes, scores in zip(pred_boxes[-len(pred_scores):],
                                     pred_scores):
                center_loss += centerness_loss(scores.flatten(),
                                               boxes.view(-1, 4),
                                               gt_boxes).mean()
        ...
        if pred_masks is not None and giou_loss.item() < 1.0:
            mask_loss, mask_metric = self.mask_loss(
                ...
                data['search_target_masks'].flatten(0, 1))
