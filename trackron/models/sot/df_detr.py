...
class DeformableTransformerHead(nn.Module):
    ...
    def forward(self,
        ...
        #### process feat
        temp_srcs, temp_masks, temp_posembs = self.get_process_features(
            template_features, template_features, template_masks)
        search_srcs, search_masks, search_posembs = self.get_process_features(
            search_features, template_features, search_masks)

        # TRACED 得到 送入 Track Transformer 的 search feature
        ...
        # encoder
        temp_memory = self.encoder(temp_srcs, spatial_shapes, valid_ratios,
                                   temp_posembs, temp_masks)
        search_memory = self.encoder(search_srcs, spatial_shapes, valid_ratios,
                                     search_posembs, search_masks)

        tgt = self.extract_roi_target(temp_memory, template_boxes,
                                      spatial_shapes)
        reference = box_convert(template_boxes, 'xyxy', 'cxcywh') / 352
        reference = repeat(reference, 'b c -> b l c',
                           l=len(spatial_shapes)).detach()
        reference = torch.full_like(reference, 0.5)
        query_embed = None

        # decoder
        temp_target, _ = self.decoder(tgt, reference, temp_memory,
                                      spatial_shapes, valid_ratios,
                                      query_embed, temp_masks)
        search_target, _ = self.decoder(tgt, reference, search_memory,
                                        spatial_shapes, valid_ratios,
                                        query_embed, search_masks)
        memory = torch.stack([temp_memory, search_memory], dim=1).flatten(0, 1)
        target_feat = torch.stack([temp_target, search_target],
                                  dim=1).flatten(0, 1)
        # Run the box decoder module
        # pred_boxes = self.decode_box(search_memory, search_target, spatial_shapes, layer=0)
        pred_boxes = self.decode_box(memory,
                                     target_feat,
                                     spatial_shapes,
                                     layer=0)
    ...
    def refine_boxes(self, search_feat, target_feat, pred_boxes):
        ...
        refine_proposals, _ = self.refine_head(search_feat, target_feat,
                                               proposals)
        refine_proposals = [
            rearrange(boxes, 'B N C -> N B C') / sz / 16
            for boxes in [proposals] + refine_proposals
        ]
    ...
    def decode_box(self,
        ...
        if self.refine_head is not None:
            pred_boxes = self.refine_boxes(srcs[layer], target_feat,
                                           pred_boxes)
    ...
    def get_process_features(self, feat1, feat2, feat_mask):
        ...
        if len(self.feature_layers) < len(self.input_proj):
            ...
            mask, posemb = self.get_feature_maskposemb(srcs[-1],
                                                       feat_mask[None])
            masks += [mask]
            posembs += [posemb]
    ...
    def track_init(self, features, boxes, masks=None):
        srcs, masks, posembs = self.get_process_features(
            features, features, masks)

        ### flatten feats for attention
        self.spatial_shapes = torch.tensor(
            [list(src.shape[-2:]) for src in srcs],
            device=boxes.device,
            dtype=torch.long)
        self.valid_ratios = torch.stack(
            [self.get_valid_ratio(m) for m in masks], 1)
        srcs, masks, posembs = self.flatten_feats(srcs, masks, posembs)

        # encoder
        memory = self.encoder(srcs, self.spatial_shapes, self.valid_ratios,
                              posembs, masks)
        target_features = self.extract_roi_target(memory, boxes,
                                                  self.spatial_shapes)
    ...
    def track(self, features, masks, ref_info):
        ...
        tgt, reference = ref_info['target_features'], ref_info['reference']
        srcs, masks, posembs = self.get_process_features(
            features, template_features, masks)
        srcs, masks, posembs = self.flatten_feats(srcs, masks, posembs)
        memory = self.encoder(srcs, self.spatial_shapes, self.valid_ratios,
                              posembs, masks)
        # decoder
        query_embed = None
        target_feat, reference = self.decoder(tgt, reference, srcs,
                                              self.spatial_shapes,
                                              self.valid_ratios, query_embed,
                                              masks)
        # Run the box decoder module
        pred_boxes = self.decode_box(memory,
                                     target_feat,
