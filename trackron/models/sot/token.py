...
class TokenHead(nn.Module):
    ...
    def forward(self,
        ...
        template_features = self.input_proj(
            template_features[self.feature_layer])
        ...
        target_feature, target_mask, target_posemb = self.extract_target_feat(
            template_features, template_boxes, template_masks,
            template_posembs)
        ...
        search_features = torch.stack([template_features, search_features],
                                      dim=1).flatten(0, 1)
        search_masks = torch.stack([template_masks, search_masks],
                                   dim=1).flatten(0, 1)
        search_posembs = torch.stack([template_posembs, search_posembs],
                                     dim=1).flatten(0, 1)
        features, masks, posembs = self.fuse_feature(
            target_feature.flatten(-2), target_mask.flatten(-2),
            target_posemb.flatten(-2), search_features.flatten(-2),
            search_masks.flatten(-2), search_posembs.flatten(-2))
        ...
        if self.refine_head is not None:
            pred_boxes = self.refine_boxes(search_feat, target_feat,
                                           pred_boxes)
    ...
    def fuse_feature(self, target_features, target_masks, target_posembs,
                     search_features, search_masks, search_posembs):
    ...
    def track_init(self, features, boxes, masks=None):
        ...
        target_feature, target_mask, target_posemb = self.extract_target_feat(
            features, boxes, masks, posembs)

        return {
            "target_feature": target_feature.flatten(-2),
            "target_mask": target_mask.flatten(-2),
            "target_posemb": target_posemb.flatten(-2)
        }

    def track(self, features, masks, ref_info):
        cur_features = self.input_proj(features[self.feature_layer])
        B, C, H, W = cur_features.shape
        cur_masks, cur_posembs = self.get_feature_maskposemb(
            cur_features, masks)
        target_feature = ref_info['target_feature']
        target_mask = ref_info['target_mask']
        target_posemb = ref_info['target_posemb']
        features, masks, posembs = self.fuse_feature(target_feature,
                                                     target_mask,
                                                     target_posemb,
                                                     cur_features.flatten(-2),
                                                     cur_masks.flatten(-2),
                                                     cur_posembs.flatten(-2))
        features = self.encoder(features,
                                src_key_padding_mask=masks,
                                pos=posembs)
        target_feat = rearrange(features[:-H * W], 'n b c -> b (c n)')[None]
        ...
        cur_target_feature, cur_target_mask, cur_target_posemb = self.extract_target_feat(
            cur_features, boxes, cur_masks, cur_posembs)
