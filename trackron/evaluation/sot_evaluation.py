...
_PALETTE = (np.array(sns.color_palette(n_colors=256)) *
            255).astype('uint8').ravel()
...
def calc_iou_overlap(pred_bb, anno_bb):
    ...
    union = pred_bb[:, 2:].prod(dim=1) + anno_bb[:,
                                                 2:].prod(dim=1) - intersection
...
def calc_seq_err_robust(pred_bb, anno_bb, dataset="otb", target_visible=None):
    ...
    if (pred_bb[:, 2:] == 0.0).any():
        for i in range(1, pred_bb.shape[0]):
            if (pred_bb[i, 2:]
                    == 0.0).any() and not torch.isnan(anno_bb[i, :]).any():
...
class SOTEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        if "target_bbox" in outputs:
            target_bbox = torch.tensor(outputs["target_bbox"],
                                       dtype=torch.float32)
    ...
    def evaluate(self):
        ...
        if len(predictions) == 0:
            self._logger.warning(
                "[SOT evaluator] Did not receive valid predictions.")
            return {}
    ...
    def _eval_tracking_boxes(self, predictions):
        ...
        threshold_set_center_norm = torch.arange(0, 51,
        ...
        ave_success_rate_plot_overlap = torch.zeros(
            (len(predictions), threshold_set_overlap.numel()),
            dtype=torch.float32)
        ave_success_rate_plot_center = torch.zeros(
            (len(predictions), threshold_set_center.numel()),
            dtype=torch.float32)
        ...
        for seq_id, (pred_bb, anno_bb, target_visible) in enumerate(
            ...
            ave_success_rate_plot_overlap[seq_id, :] = (err_overlap.view(
                -1, 1) > threshold_set_overlap.view(
                    1, -1)).sum(0).float() / seq_length * 100
            ave_success_rate_plot_center[seq_id, :] = (err_center.view(
                -1, 1) <= threshold_set_center.view(
                    1, -1)).sum(0).float() / seq_length * 100
            ave_success_rate_plot_center_norm[
                seq_id, :] = (err_center_normalized.view(-1, 1) <=
                              threshold_set_center_norm.view(
                                  1, -1)).sum(0).float() / seq_length * 100
        ...
        sot_results = {
            ...
            'NormPrecision':
            ave_success_rate_plot_center_norm.mean(0)[20].item()
        }
